...
def mk_pt_pt (O A : P) (h : A ≠ O) : Circle P where
  center := O
  radius := (DSEG O A).length
  rad_pos := (DSeg.nontriv_iff_length_pos _).mp h
...
def power (ω : Circle P) (p : P) : ℝ := (DSEG ω.center p).length ^ 2 - ω.radius ^ 2

def lies_inside_circle (p : P) (ω : Circle P) : Prop := (DSEG ω.center p).length ≤  ω.radius

def lies_on_circle (p : P) (ω : Circle P) : Prop := (DSEG ω.center p).length = ω.radius

def lies_interior_circle (p : P) (ω : Circle P) : Prop := (DSEG ω.center p).length < ω.radius

def lies_outside_circle (p : P) (ω : Circle P) : Prop := ω.radius < (DSEG ω.center p).length
...
def dseg_lies_inside_circle (l : DSeg P) (ω : Circle P) : Prop := l.source LiesInCir ω ∧ l.target LiesInCir ω

end Circle

scoped infix : 50 "DSegInCir" => Circle.dseg_lies_inside_circle

namespace Circle

-- very hard question: if a segment lies inside a circle, then the interior point of a

theorem pt_lies_inside_circle_of_dseg_inside_circle {l : DSeg P} {ω : Circle P} (h : l DSegInCir ω) {p : P} (lieson : p LiesOnIntDSeg l) : p LiesIntCir ω := sorry
