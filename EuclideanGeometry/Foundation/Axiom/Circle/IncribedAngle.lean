...
theorem antipode_iff_colinear {A B : P} {ω : Circle P} (h : B ≠ A) (h₁ : A LiesOn ω) (h₂ : B LiesOn ω) : IsAntipode A B h h₁ h₂ ↔ colinear ω.center A B := by
  ...
  intro hh
  show ∠ A ω.center B (Circle.pt_lieson_ne_center h₁) (Circle.pt_lieson_ne_center h₂) = π
  have hl : B LiesOn (LIN ω.center A (Circle.pt_lieson_ne_center h₁)) := Line.pt_pt_maximal _ hh
  have heq₁ : VEC A ω.center = VEC ω.center B := by
    apply distinct_pts_same_dist_vec_eq (Circle.pt_lieson_ne_center h₁) h hl
    rw [h₁, h₂]
  ...
  have hli : ω.center LiesInt (SEG_nd A B h) := by
...
theorem mk_pt_pt_diam_isantipode {A B : P} (h : B ≠ A) : IsAntipode A B h (Circle.mk_pt_pt_diam_fst_lieson h) (Circle.mk_pt_pt_diam_snd_lieson h) := by
  have hc : colinear (SEG A B).midpoint A B := by
    apply perm_colinear_trd_fst_snd
    apply Line.pt_pt_linear h
    show (SEG A B).midpoint LiesOn (SEG_nd A B h).toLine
    apply SegND.lies_on_toLine_of_lie_on
    apply Seg.midpt_lies_on
  apply (antipode_iff_colinear h (Circle.mk_pt_pt_diam_fst_lieson h) (Circle.mk_pt_pt_diam_snd_lieson h)).mpr hc
...
theorem inscribed_angle_of_diameter_eq_mod_pi_pt_pt_pt {A B C : P} {ω : Circle P} (hne₁ : A ≠ B) (hne₂ : B ≠ C) (hne₃ : C ≠ A) (h₁ : A LiesOn ω) (h₂ : B LiesOn ω) (h₃ : C LiesOn ω) (h : Arc.IsAntipode A B hne₁.symm h₁ h₂) : ∠ A C B hne₃.symm hne₂ = ∡[π / 2] := by
  let β : Arc P := ARC A B hne₁.symm h₁ h₂
  have hh₁ : C LiesOn β.circle := h₃
  have hh₂ : Arc.IsAntipode β.source β.target β.endpts_ne β.ison.1 β.ison.2 := h
  have hh₃ : Arc.Isnot_arc_endpts C β := ⟨hne₃,hne₂.symm⟩
