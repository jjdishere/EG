...
lemma tangent_length_sq_eq_power {p : P} {l : DirLine P} {ω : Circle P} (h₁ : l Tangent ω) (h₂ : p LiesOn l) : (dist p (DirLC_Tangent_pt h₁)) ^ 2 = power ω p := by
  calc
    _ = (dist p (perp_foot ω.center l)) ^ 2 := by rw [DirLC_tangent_pt_eq_perp_foot]
