...
theorem line_of_pt_pt_eq_rev : LIN A B h = LIN B A h.symm :=
  (Quotient.eq (r := same_extn_line.setoid)).mpr ⟨Ray.toProj_eq_toProj_of_mk_pt_pt h, Or.inl (Ray.snd_pt_lies_on_mk_pt_pt h)⟩
...
theorem mk_pt_proj_eq {A : P} {l : Line P} (h : A LiesOn l) : Line.mk_pt_proj A l.toProj = l := by
  revert l
  rintro ⟨r⟩ h
  exact (@Quotient.map_mk _ _ PM.con.toSetoid same_extn_line.setoid (fun x : _ ↦ Ray.mk A x) (same_extn_line_of_PM A) r.2).trans
...
theorem Ray.toLine_eq_rev_toLine : ray.toLine = ray.reverse.toLine :=
  (Quotient.eq (r := same_extn_line.setoid)).mpr ⟨toProj_of_rev_eq_toProj.symm, Or.inr source_lies_on⟩
...
theorem toLine_eq_extn_toLine : seg_nd.toLine = seg_nd.extension.toLine := (Quotient.eq (r := same_extn_line.setoid)).mpr ⟨by
