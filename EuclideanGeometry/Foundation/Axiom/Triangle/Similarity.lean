...
theorem ratio₂ : h.ratio = tr₁.edge₂.length / tr₂.edge₂.length := by
  ...
  have s₁₂: tr₁.edge₁.length / tr₁.edge₂.length = (sin (Angle.value tr₂.angle₁)) / (sin (Angle.value tr₂.angle₂)) := by
    rw [mul_comm] at sine₁
    exact ((div_eq_div_iff  (sine_ne_zero_of_nd (perm_vertices tr₂)) (length_ne_zero_iff_nd.mpr tr₁.edgeND₂.2).symm).mpr sine₁).symm
  have t₁₂: tr₂.edge₁.length / tr₂.edge₂.length = (sin (Angle.value tr₂.angle₁)) / (sin (Angle.value tr₂.angle₂)) := by
    rw [mul_comm] at sine₂
    exact ((div_eq_div_iff  (sine_ne_zero_of_nd (perm_vertices tr₂)) (length_ne_zero_iff_nd.mpr tr₂.edgeND₂.2).symm).mpr sine₂).symm
  rw [<-t₁₂] at s₁₂
  unfold ratio
  have eq := mul_eq_mul_of_div_eq_div tr₁.edge₁.length tr₂.edge₁.length (length_ne_zero_iff_nd.mpr tr₁.edgeND₂.2).symm (length_ne_zero_iff_nd.mpr tr₂.edgeND₂.2).symm s₁₂
  apply (div_eq_div_iff (length_ne_zero_iff_nd.mpr tr₂.edgeND₁.2).symm (length_ne_zero_iff_nd.mpr tr₂.edgeND₂.2).symm).mpr
  have e₂ : (tr₂.edgeND₂).1.length = tr₂.edge₂.length := rfl
  have e₁ : (tr₂.edgeND₁).1.length = tr₂.edge₁.length := rfl
  have e₂' : (tr₁.edgeND₂).1.length = tr₁.edge₂.length := rfl
  rw [e₂, e₂'] at eq
...
theorem ratio₂ : h.ratio = tr₁.edge₂.length / tr₂.edge₂.length := by
  ...
  have s₁₂: tr₁.edge₁.length / tr₁.edge₂.length = (sin (- Angle.value tr₂.angle₁)) / (sin (-Angle.value tr₂.angle₂)) := by
    rw [mul_comm] at sine₁
    exact ((div_eq_div_iff nd (length_ne_zero_iff_nd.mpr tr₁.edgeND₂.2).symm).mpr sine₁).symm
  have t₁₂: tr₂.edge₁.length / tr₂.edge₂.length = (sin (Angle.value tr₂.angle₁)) / (sin (Angle.value tr₂.angle₂)) := by
    rw [mul_comm] at sine₂
    exact ((div_eq_div_iff (sine_ne_zero_of_nd (perm_vertices tr₂)) (length_ne_zero_iff_nd.mpr tr₂.edgeND₂.2).symm).mpr sine₂).symm
...
theorem sim_of_SAS (tr₁ tr₂ : TriangleND P) (e : tr₁.edge₂.length / tr₂.edge₂.length = tr₁.edge₃.length / tr₂.edge₃.length) (a : tr₁.angle₁.value = tr₂.angle₁.value): tr₁ ∼ tr₂ := by
  have eq : tr₁.edge₂.length * tr₂.edge₃.length = tr₁.edge₃.length * tr₂.edge₂.length := by
    exact (div_eq_div_iff (length_ne_zero_iff_nd.mpr tr₂.edgeND₂.2).symm (length_ne_zero_iff_nd.mpr tr₂.edgeND₃.2).symm).mp e
  rw [mul_comm tr₁.edge₃.length  tr₂.edge₂.length] at eq
  have e' : tr₁.edge₂.length / tr₁.edge₃.length = tr₂.edge₂.length / tr₂.edge₃.length := (div_eq_div_iff (length_ne_zero_iff_nd.mpr tr₁.edgeND₃.2).symm (length_ne_zero_iff_nd.mpr tr₂.edgeND₃.2).symm).mpr eq
  have sine₁ := Triangle.sine_rule₁ tr₁
  have sine₂ := Triangle.sine_rule₁ tr₂
  ...
  have ne₂₃ : sin (Angle.value tr₂.angle₃) ≠ 0 := by
    simp only [(angle_eq_angle_of_perm_vertices tr₂).2.2,ne_eq, sine_ne_zero_of_nd,
        not_false_eq_true]
  have s₁₂ : tr₁.edge₂.length / tr₁.edge₃.length = sin (Angle.value tr₁.angle₂) / sin (Angle.value tr₁.angle₃) := (div_eq_div_iff (length_ne_zero_iff_nd.mpr tr₁.edgeND₃.2).symm ne₁₃).mpr sine₁'
  have t₁₂ : tr₂.edge₂.length / tr₂.edge₃.length = sin (Angle.value tr₂.angle₂) / sin (Angle.value tr₂.angle₃) := (div_eq_div_iff (length_ne_zero_iff_nd.mpr tr₂.edgeND₃.2).symm ne₂₃).mpr sine₂'
  rw [s₁₂,t₁₂] at e'
...
theorem IsSim.congr_of_ratio_eq_one (h : IsSim tr₁ tr₂) (hr : h.ratio = 1) : tr₁.IsCongr tr₂ := by
  ...
  have := ratio₁ h
  rw [hr] at this
  exact (div_eq_one_iff_eq (length_ne_zero_iff_nd.mpr tr₂.edgeND₁.2).symm).mp this.symm
  have := ratio₂ h
  rw [hr] at this
  exact (div_eq_one_iff_eq (length_ne_zero_iff_nd.mpr tr₂.edgeND₂.2).symm).mp this.symm
  have := ratio₃ h
