...
/-
Translate useful theorems in `Mathlib.Data.Complex.Exponential` to `AngValue.sin` or `AngValue.cos`, such as the sum and difference formulas, product-to-sum formulas, and sum-to-product formulas and so on.
...
-- How to make `norm_num` or `linarith` automatically apply these theorems?
...
-- Maybe replace it by cos_pos_of_neg_pi_div_two_lt_of_lt_pi_div_two
