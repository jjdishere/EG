...
instance PScaling.setoid : Setoid Vec_nd where
  r := PScaling
...
@[simp]
theorem norm_eq_one (x : Vec_nd) : norm x.normalize.1 = 1 := sorry
...
theorem pscaling_con_le_con_ker_vec_nd_normalize : PScaling.con ≤ Con.ker Vec_nd.normalize' := by
    rw [Con.le_def]
    intro z w ⟨t, ht, e⟩
...
def toVec_nd' : (Dir →* Vec_nd) := (Con.lift PScaling.con Vec_nd.normalize') pscaling_con_le_con_ker_vec_nd_normalize

def toVec_nd (x : Dir) : Vec_nd := (Quotient.lift (s := PScaling.con.toSetoid) Vec_nd.normalize) pscaling_con_le_con_ker_vec_nd_normalize x

theorem test : toVec_nd'.toFun = toVec_nd := rfl

def toVec (x : Dir) : Vec := x.toVec_nd.1

theorem unit (x : Dir) : inner x.toVec x.toVec = (1 : ℝ) := by
  sorry

@[simp]
theorem norm_of_dir_toVec_eq_one (x : Dir) : Vec.norm x.toVec_nd.1 = 1 := by
  revert x
  unfold Dir Con.Quotient
  rw [@forall_quotient_iff _ PScaling.con.toSetoid (fun _ : Dir => _)]
  intro x
  unfold toVec_nd
  simp only [ne_eq, OneHom.toFun_eq_coe, MonoidHom.toOneHom_coe, Quotient.lift_mk]
  exact norm_eq_one _
...
@[simp]
theorem toVec_nd_neg_eq_neg_toVec_nd (x : Dir) : Dir.toVec_nd (-x) = -(Dir.toVec_nd x) := by 
  unfold toVec_nd 
  revert x
  unfold Dir Con.Quotient
  rw [@forall_quotient_iff _ PScaling.con.toSetoid (fun _ : Dir => _)]
...
theorem toVec_neg_eq_neg_toVec (x : Dir) : (-x).toVec = -(x.toVec) := by 
  unfold toVec
  simp only [ne_eq, toVec_nd_neg_eq_neg_toVec_nd, fst_neg_Vec_nd_is_neg_fst_Vec_nd]
