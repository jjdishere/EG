...
class Carrier (P: Type*) [EuclideanPlane P] (α : Type*) where
  carrier : α → Set P

class Interior (P: Type*) [EuclideanPlane P] (α : Type*) where
  interior : α → Set P


def lies_on {P : Type*} [EuclideanPlane P] {α : Type*} [Carrier P α] (A : P) (F : α) := A ∈ (Carrier.carrier F)

def lies_int {P : Type*} [EuclideanPlane P] {α : Type*} [Interior P α] (A : P) (F : α) := A ∈ (Interior.interior F)

-- def lies_in {P : Type*} [EuclideanPlane P] {α : Type*} [Carrier P α] [Interior P α] (A : P) (F : α) : Prop := lies_int A F ∨ lies_on A F

def is_inx {P : Type*} [EuclideanPlane P] {α β: Type*} [Carrier P α] [Carrier P β] (A : P) (F : α) (G : β) := A ∈ (Carrier.carrier F) ∧ A ∈ (Carrier.carrier G)

theorem is_inx.symm {P : Type*} [EuclideanPlane P] {α β: Type*} [Carrier P α] [Carrier P β] {A : P} {F : α} {G : β} (h : is_inx A F G) : is_inx A G F := And.symm h
...
theorem ne_of_lieson_and_not_lieson {P : Type*} [EuclideanPlane P] {α : Type*} [Carrier P α] {F : α} {X Y : P} (hx : X LiesOn F) (hy : ¬ Y LiesOn F) : X ≠ Y := by
  by_contra h
  rw [h] at hx
  tauto

theorem ne_of_liesint_and_not_liesint {P : Type*} [EuclideanPlane P] {α : Type*} [Interior P α] {F : α} {X Y : P} (hx : X LiesInt F) (hy : ¬ Y LiesInt F) : X ≠ Y := by
  by_contra h
  rw [h] at hx
  tauto
end compatibility
...
def concurrent {P : Type*} [EuclideanPlane P] {α β γ: Type*} [Carrier P α] [Carrier P β] [Carrier P γ] (A : P) (F : α) (G : β) (H : γ) : Prop := A LiesOn F ∧ A LiesOn G ∧ A LiesOn H

class Convex2D (P: Type*) [EuclideanPlane P] (α : Type*) extends (Carrier P α), (Interior P α) where
