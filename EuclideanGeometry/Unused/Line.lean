...
def mk_pt_pt (A B : P) (h : B ≠ A) : Line P where
  ...
  maximal x y := by
    ...
    rcases (eq_mul_vec_iff_collinear_of_ne hne).1 c with ⟨t, ht⟩
    use tx + t * (ty - tx)
...
theorem vec_eq_smul_vec_of_lies_on {l : Line P} {A B X Y : P} (ha : A LiesOn l) (hb : B LiesOn l) (hx : X LiesOn l) (hy : Y LiesOn l) (hab : B ≠ A) : ∃ t : ℝ, VEC X Y = t • VEC A B := by
  rcases (eq_mul_vec_iff_collinear_of_ne hab).1 (Line.linear A B X ha hb hx) with ⟨t₁, e₁⟩
  rcases (eq_mul_vec_iff_collinear_of_ne hab).1 (Line.linear A B Y ha hb hy) with ⟨t₂, e₂⟩
  use t₂ - t₁
...
theorem eq_line_of_pt_pt_of_ne {A B : P} {l : Line P} (h : B ≠ A) (ha : A LiesOn l) (hb : B LiesOn l) : LIN A B h = l := by
  ...
  constructor
  intro hx
  apply (lies_on_iff_collinear_of_ne_lies_on_lies_on h ha hb X).2
  exact (lies_on_line_of_pt_pt_iff_collinear h X).1 hx
  intro hx
  exact (lies_on_line_of_pt_pt_iff_collinear h X).2 ((lies_on_iff_collinear_of_ne_lies_on_lies_on h ha hb X).1 hx)
