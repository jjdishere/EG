...
lemma e_ne_B : E ≠ B := by -- for $E \ne B$
  have b_not_lieson_ac := (Line.lies_on_line_of_pt_pt_iff_collinear (_h := ⟨ c_ne_a (hnd := hnd)⟩) B).mp.mt (collinear132.mt hnd) -- $B$ doesn't lies on line $AC$ because $A, B, C$ not collinear
